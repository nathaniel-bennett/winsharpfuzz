...
namespace WinSharpFuzz
{
	...
	public static partial class Fuzzer
	{
		...
		/// True if <see cref="WinSharpFuzz.Common.Trace.OnBranch"/> callback
		...
		private static TypeDefUser GenerateTraceType(ModuleDefMD mod)
		{
			var traceType = new TypeDefUser(
				typeof(WinSharpFuzz.Common.Trace).FullName,
		...
		{
			Console.Error.WriteLine("The use of AFL (or WinAFL) is not currently supported for this Windows port of SharpFuzz. Please use libFuzzer (Fuzzer.LibFuzzer.Run) instead.");
